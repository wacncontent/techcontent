...
转到[创建和部署 blink 应用程序](./iot-hub-raspberry-pi-kit-node-lesson1-deploy-blink-app.md)。
...
转到[创建 Azure 函数应用和 Azure 存储帐户](./iot-hub-raspberry-pi-kit-node-lesson3-deploy-resource-manager-template.md)。
...
转到[运行示例应用程序，以便发送从设备到云的消息](./iot-hub-raspberry-pi-kit-node-lesson3-run-azure-blink.md)。
...
转到[读取保存在 Azure 存储中的消息](./iot-hub-raspberry-pi-kit-node-lesson3-read-table-storage.md)。
...
转到[运行示例应用程序，接收从云到设备的消息](./iot-hub-raspberry-pi-kit-node-lesson4-send-cloud-to-device-messages.md)。
...
转到[可选部分：更改 LED 的开关行为](./iot-hub-raspberry-pi-kit-node-lesson4-change-led-behavior.md)。

## 故障排除
如果在课程中碰到问题，可参阅[故障排除](./iot-hub-raspberry-pi-kit-node-troubleshooting.md)一文以获取解决方案。
