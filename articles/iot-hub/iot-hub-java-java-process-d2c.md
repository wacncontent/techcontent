...
2. 向 **App** 类添加以下嵌套类：
    ...
    > [!NOTE]
    > 有必要在设备代码中创建用于删除重复交互式消息的 **MessageId**。间歇性网络通信或其他故障可能会导致多次重复传输来自该设备的相同消息。还可将 UUID 换用为语义消息 ID，如相关消息数据字段的哈希。
...
> [!NOTE]
> 本教程使用单个 Azure 存储帐户写入从 IoT 中心检索的所有消息。若要确定解决方案中是否需使用多个 Azure 存储帐户，请参阅 [Azure 存储可缩放性指导原则]。
