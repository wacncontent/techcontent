...
>[!NOTE] 如果 IP 地址可用于测试故障转移网络，则在测试故障转移期间指定给虚拟机的 IP 地址与在运行计划或非计划故障转移时收到的 IP 地址相同。如果这个相同的 IP 在测试性故障转移网络中不可用，那么虚拟机会收到可在测试故障转移网络中使用的其他 IP 地址。
...
3. 如果您要从 Azure 故障回复，请在“数据同步”中选择设置：
	...
	> [!NOTE] 如果已在 Azure 上运行了一段时间（一个月或以上）或已删除本地 VM，我们建议你使用此选项。此选项不会执行任何校验和计算。
...
如果您在 [Hyper-V 站点与 Azure](./site-recovery-hyper-v-site-to-azure.md) 之间部署了保护，则可以从 Azure 故障回复到备用本地位置。如果你需要设置新的本地硬件，此功能将十分有用。下面介绍了操作方法。
