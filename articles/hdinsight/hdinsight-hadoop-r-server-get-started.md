...
## 先决条件
* **Azure 订阅**：在开始学习本教程之前，必须有一个 Azure 订阅。请参阅 [Get Azure trial](https://www.azure.cn/pricing/1rmb-trial/)（获取 Azure 试用版）了解详细信息。
...
## 创建群集
> [!NOTE]
本文档中的步骤将指导用户完成如何使用基本配置信息在 HDInsight 群集上创建 R Server。有关其他群集配置设置（例如，添加其他存储帐户、使用 Azure 虚拟网络或创建 Hive 元存储）的信息，请参阅 [Create Linux-based HDInsight clusters](./hdinsight-hadoop-provision-linux-clusters.md)（创建基于 Linux 的 HDInsight 群集）。
>
> 
