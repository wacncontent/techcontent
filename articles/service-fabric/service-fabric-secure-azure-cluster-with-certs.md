---
title: 使用证书保护 Service Fabric 群集 | Azure
...
有关 Service Fabric 如何使用 X.509 证书的详细信息，请参阅 [Cluster security scenarios](./service-fabric-cluster-security.md)（群集安全方案）。
...
### 步骤 2.1
确保 Azure PowerShell 1.0+ 已安装在计算机上。如果尚未安装，请按照 [How to install and configure Azure PowerShell.](../powershell-install-configure.md)（如何安装和配置 Azure PowerShell）中所述的步骤进行安装。
...
你可以使用 [Azure Resource Manager 模板](./service-fabric-cluster-creation-via-arm.md)来为群集中的每个节点类型进行安全配置。
...
<!--
## 后续步骤
在群集上配置证书安全性之后，请继续执行[步骤 4：完成群集创建](./service-fabric-cluster-creation-via-portal.md#step-4--complete-the-cluster-creation)中的群集创建过程。

为群集创建证书安全性之后，可以[更新证书](./service-fabric-cluster-security-update-certs-azure.md)。
-->
