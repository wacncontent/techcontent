...
> [!NOTE]<p>由于在包含父目录时将以递归方式包含子目录，因此请仅指定父目录。此外，不要指定其任何子目录。<p>目前，Azure 导入/导出工具存在以下限制：如果目录包含的数据超过硬盘驱动器可包含的数据，该目录需要分解为小目录。例如，如果目录包含 2.5TB 数据，而硬盘驱动器的容量仅为 2TB，则需要将此 2.5TB 目录分解为多个小目录。该工具的更高版本中将解除该限制。
...
> [!NOTE]如果有多台计算机满足 [Setting Up the Azure Import-Export Tool](./storage-import-export-tool-setup-v1.md)（设置 Azure 导入/导出工具）中所列的要求，则可以通过在每台计算机上运行此工具的实例，将数据并行复制到多个硬盘驱动器。
...
> [!WARNING] 如果为第一个复制会话指定 **/format** 参数，将格式化驱动器并清除该驱动器上的所有数据。建议仅对复制会话使用空白驱动器。
