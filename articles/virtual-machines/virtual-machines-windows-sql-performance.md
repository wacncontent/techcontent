...
创建 SQL Server 映像时，[请考虑在 Azure 门户预览中设置 VM](./virtual-machines-windows-portal-sql-server-provision.md)。使用 Resource Manager 在门户中设置的 SQL Server VM 将实施所有这些最佳实践，包括存储配置。
...
|区域|优化|
|---|---|
|[VM 大小](#vm-size-guidance)|SQL Enterprise 版本为 [DS3](./virtual-machines-windows-sizes.md#standard-tier-ds-series) 或更高。SQL Standard 和 Web 版本为 <br/><br/>[DS2](./virtual-machines-windows-sizes.md#standard-tier-ds-series) 或更高。|
...
|[磁盘](#disks-guidance)|至少使用 2 个 [P30 磁盘](../storage/storage-premium-storage.md#scalability-and-performance-targets-when-using-premium-storage)（1 个用于日志文件；1 个用于数据文件和 TempDB）。<br/><br/>避免将操作系统磁盘或临时磁盘用于数据库存储或日志记录。<br/><br/>在托管数据文件和 TempDB 的磁盘上启用读缓存。<br/><br/>请勿在托管日志文件的磁盘上启用缓存。<br/><br/>重要说明：在更改 Azure VM 磁盘的缓存设置时，请停止 SQL Server 服务。<br/><br/>条带化多个 Azure 数据磁盘以获得更高的 IO 吞吐量。<br/><br/>使用文档中记录的分配大小格式化。|
