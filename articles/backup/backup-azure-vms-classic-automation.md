...
> [!NOTE] 在开始之前，请阅读使用 Azure 备份所需的[系统必备组件](./backup-azure-vms-prepare.md)，以及当前 VM 备份解决方案的[限制](./backup-azure-vms-prepare.md#limitations/)。
...
如果你更愿意使用 PowerShell 来处理 Azure 资源，则请查看有关如何保护 Windows Server 的 PowerShell 文章：[为 Windows Server 部署和管理备份](./backup-client-automation-classic.md)。此外还有一篇有关如何管理 DPM 备份的 PowerShell 文章：[为 DPM 部署和管理备份](./backup-dpm-automation-classic.md)。这两篇文章都为资源管理器部署和经典部署提供了一个版本。
