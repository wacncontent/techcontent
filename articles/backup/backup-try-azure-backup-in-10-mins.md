...
5. 在“快速启动”页中，单击“下载保管库凭据”。
    ...
    >[!NOTE] 保管库凭据文件仅在注册工作流中使用，48 小时后过期。
...
7. 在“加密设置”屏幕中，你可以生成一个通行短语，或者提供一个通行短语（最少包含 16 个字符）。请记住将通行短语保存在安全位置。
    ...
    > [!WARNING] 如果你丢失或忘记了通行短语，Microsoft 无法帮助你恢复备份的数据。加密通行短语由最终用户拥有，Microsoft 看不到最终用户所用的通行短语。请将该文件保存在安全位置，因为在恢复操作期间需要用到它。
...
5. 选择备份副本的“保留策略”。你可以根据需要修改每日、每周、每月和每年保留策略。
    ...
    >[!NOTE] [此文](./backup-azure-backup-cloud-as-tape.md)中详细介绍了备份计划。
