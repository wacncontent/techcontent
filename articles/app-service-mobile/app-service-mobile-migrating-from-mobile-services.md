...
> [!TIP] 应用服务标准层包含可能要使用的多种功能的访问权限，包括[过渡槽]、自动备份和自动缩放。可以在相关位置查看新功能。
...
> [!TIP]  使用 Azure 应用服务的优势之一是可以在相同站点上运行网站和移动服务。有关详细信息，请参阅[后续步骤](#next-steps)部分。
...
> [!TIP]  有两项应用程序设置的值相同。例如，可能会出现 _ApplicationKey_ 和 _MS\_ApplicationKey_。同时更新这两个应用程序设置。
...
> [!NOTE] 如果通知中心是“混合”类型，则它不可见。“混合”类型的通知中心同时利用通知中心功能和旧式服务总线功能。[转换混合命名空间]，然后继续。转换完成后，通知中心将出现在 [Azure 门户预览]中。

> [!TIP] [Azure 门户预览]中的通知中心管理功能仍以预览版提供。仍可使用 [Azure 经典管理门户]管理所有通知中心。
